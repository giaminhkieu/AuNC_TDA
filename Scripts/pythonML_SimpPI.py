...
target_variable = "gap"
model_name = "010823_SimpPI_" + target_variable

for i in range(len(DatasetAuNC)):
  arr = []
  arr.append(DatasetAuNC["core"][i])
  arr.append(DatasetAuNC["Tetrahedral_count"][i])
  arr.append(DatasetAuNC["Unconnected_triangles_count"][i])
  arr.append(DatasetAuNC["Triangles_with_1_shared_vertex_count"][i])
  arr.append(DatasetAuNC["Triangles_with_2_shared_vertices_count"][i])
  arr = np.asarray(arr)
  x1.append(arr)

for i in range(len(DatasetAuNC)):
  arr = DatasetAuNC["PersImg"][i]
  arr = np.asarray(arr)
  x2.append(arr.reshape(100,100,1))
...
for test_index in range(len(x1)):
  ...
  # dense_4 = Dense(1, activation='relu')(dense_3)

  # Concatenate models 1,2 and 3
  input = Concatenate()([dense_1_extra, dense_3])
  # input = Concatenate()([dense_1_extra, Resnet, input3])
 ...
 #           verbose=1)
  ...
  y_pred = model.predict([np.asarray([x1_test]), np.asarray([x2_test])])
...
size = len(y)
print(80*"-")
print("Simp+PI input model with EarlyStopping,", target_variable)
print(80*"-")
print("Model structure: ")
print("5,5,1 || (16,3,3), (2,2), (16,3,3), (2,2), flatten, 32, 16 || NA || 64,1")
print("Hyperparameters: ")
print("Epochs: 500, ES = 40 patience Opt: Adam (MAE), LR: 0.0005, batch size: 24")
print("Validation split: N/A")
print(80*"-")
print("Test results")
print(f"Mean Absolute Error = {TotalError/size}")
print("RMSE =", np.sqrt(MSE/size))
