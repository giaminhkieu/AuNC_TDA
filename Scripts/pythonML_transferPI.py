...
'''
Initializing and data appending for PI only Model
'''
import random
...
target_variable = "mu"
model_name = "010823_transferPI_" + target_variable
...
'''
Transfer-learning implemented PI only model
'''
# Hyperparams
...
PATIENCE = 40
VALIDATION_SIZE = 15
DENSE_LAYER_1_SIZE = 64
DENSE_LAYER_2_SIZE = 16
...
for test_index in range(len(x1)):
  ...
  random.seed(42)
  random_index = [random.randint(0, 211) for i in range(VALIDATION_SIZE)]
  ...
  base_model = load_model('010823_transferTM_' + 'dipTotal' + '.h5')
  for layer in base_model.layers[:-2]:
      layer.trainable = False
  new_layer = base_model(input2)
  flatten = Flatten()(new_layer)
  x = Dense(DENSE_LAYER_1_SIZE, activation='relu')(flatten)
  x = Dense(1)(x)
  ...
  y_pred = model.predict([np.asarray([x2_test])])
...
size = len(y)
print(80*"-")
print("Hyperparams: Lr =", LEARNING_RATE, "batch =", BATCH_SIZE, "Epochs=", EPOCHS, "patience=", PATIENCE, "val size=", VALIDATION_SIZE)
print("Structure: Transfer learning", DENSE_LAYER_1_SIZE, DENSE_LAYER_2_SIZE)
